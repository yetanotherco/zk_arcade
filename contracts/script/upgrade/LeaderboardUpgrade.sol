...
contract LeaderboardUpgrader is Script {
    function run(string memory leaderboardDeploymentFilePath) external returns (address, address) {
        string memory leaderboard_deployment_file = vm.readFile(leaderboardDeploymentFilePath);

        vm.startBroadcast();

        Leaderboard LeaderboardProxy =
            Leaderboard(payable(stdJson.readAddress(leaderboard_deployment_file, ".addresses.proxy")));
