...
# Deploy NFT Contract
CMD="forge script script/upgrade/NftUpgrade.sol:NftUpgrader \
    $NFT_OUTPUT_PATH \
    --rpc-url $RPC_URL \
...
jq --arg nft_implementation "$nft_implementation" '.addresses.implementation = $nft_implementation' $NFT_OUTPUT_PATH > "$NFT_OUTPUT_PATH.temp"

# Replace the original file with the temporary file
mv "$NFT_OUTPUT_PATH.temp" $NFT_OUTPUT_PATH

# Delete the temporary file
rm -f "$NFT_OUTPUT_PATH.temp"
