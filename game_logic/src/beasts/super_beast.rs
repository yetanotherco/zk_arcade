...
mod tests {
    ...
    fn super_beast_new_test() {
        ...
        assert_eq!(
            ...
            "We have created a new instance of SuperBeast"
        );
    }
    ...
    fn astar_direct_path_test() {
        ...
        assert_eq!(
            unwrapped_path[0], beast_position,
            "Path should start from the beast's position"
        );
        assert_eq!(
            unwrapped_path.last().unwrap(),
            &player_position,
            "Path should end at the player's position"
        );
    }
    ...
    fn advance_player_adjacent_test() {
        ...
        assert_eq!(
            ...
            Tile::SuperBeast,
            "The previous player tile is now the beast tile"
        );
        assert_eq!(
            board[&Coord { column: 1, row: 1 }],
            Tile::Empty,
            "The previous player tile is now cleared"
        );
        ...
        assert_eq!(
            ...
            "The player was killed by the beast"
        );
        assert_eq!(
            beast.position, player_position,
            "The beast should move to the player's position"
        );
        assert_eq!(
    ...
    fn advance_move_towards_player_test() {
        ...
        assert_eq!(
            ...
            Tile::SuperBeast,
            "Beast tile was placed correctly"
        );
        assert_eq!(
            board[&beast_position],
            Tile::Empty,
            "Beast tile was replaced correctly"
        );
        ...
        assert!(
            ...
            "New position should be closer to player"
        );
    }
    ...
    fn advance_completely_surrounded_test() {
        ...
        assert_eq!(
            beast.position, original_position,
            "Position shouldn't have changed as it's completely surrounded"
        );
        assert_eq!(
            board[&beast_position],
            Tile::SuperBeast,
    ...
    fn advance_completely_blocked_test() {
        ...
        assert_ne!(
            beast.position, original_position,
            "Position should have changed going towards the player"
        );
        assert_eq!(
            board[&original_position],
            Tile::Empty,
            "Super SuperBeasts old position has been cleared"
        );
        assert_eq!(
            board[&beast.position],
            Tile::SuperBeast,
            "Super SuperBeasts new position has been set"
        );
    }
