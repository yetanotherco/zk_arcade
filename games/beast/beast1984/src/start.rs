...
fn try_resize_console(min_width: usize, min_height: usize) {
    ...
    unsafe {
        ...
        // Use fullscreen dimensions, but ensure we meet minimum requirements
        let new_width = std::cmp::max(max_width, min_width) as i16;
        let new_height = std::cmp::max(max_height, min_height) as i16;
        
        // Set buffer size to accommodate the fullscreen size
...
pub fn start() {
    ...
    if std::env::var_os("CI").is_none() {
        if let Ok((columns, rows)) = stty::terminal_size() {
            ...
            // Always resize the console window to fullscreen on Windows
            #[cfg(windows)]
            {
                try_resize_console(min_width, min_height);
                // Re-check the size after resize
                if let Ok((new_columns, new_rows)) = stty::terminal_size() {
                    if new_columns >= min_width && new_rows >= min_height {
                        println!("Console resized to fullscreen: {}x{}", new_columns, new_rows);
                    } else {
