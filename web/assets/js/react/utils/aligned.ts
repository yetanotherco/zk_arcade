...
export const calcAlignedSavings = (
  proofs: number,
  proofType: string,
  ethPrice: number,
  gasCostGwei: number,
  ...
  if (proofType in costs) {
	const baseCostInUS = costs[proofType] * gweiPrice * gasCostGwei;
	const alignedCostInUS =
	  (alignedGasPerBatch * gweiPrice * gasCostGwei) / proofsPerBatch;

	return {
	  alignedCost: alignedCostInUS * proofs,
	  baseCost: baseCostInUS * proofs,
	  savings: (baseCostInUS - alignedCostInUS) * proofs,
	};
  }
