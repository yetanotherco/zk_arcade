...
config :zkarcade, ZkArcadeWeb.Endpoint,
  ...
  watchers: [
    esbuild: {Esbuild, :install_and_run, [:zkarcade, ~w(--sourcemap=inline --watch)]},
    tailwind: {Tailwind, :install_and_run, [:zkarcade, ~w(--watch)]}
  ]
...
config :zkarcade, ZkArcadeWeb.Endpoint,
  live_reload: [
    patterns: [
      ...
      ~r"priv/gettext/.*(po)$",
      ~r"lib/zkarcade_web/(controllers|live|components)/.*(ex|heex)$"
    ]
  ]

# Enable dev routes (for dashboard, etc). only enable behind auth
# config :zkarcade, dev_routes: true
