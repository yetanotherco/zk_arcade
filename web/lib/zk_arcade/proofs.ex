defmodule ZkArcade.Proofs do
  ...
  def get_last_claimed_proof_by_address(address) do
    Logger.info("Fetching last claimed proof for address: #{address}")
    downcased_addr = String.downcase(address)

    all_proofs = get_proofs_by_address(address)
    Logger.info("All proofs for address #{address}: #{inspect(all_proofs)}")

    Proof
    |> where([p], p.wallet_address == ^downcased_addr and p.status == "claimed")
    ...
    |> case do
      nil -> nil
      proof ->
        Logger.info("Found last claimed proof for address #{address}: #{inspect(proof)}")
        %{
          id: proof.id,
  ...
  def update_level_reached(address, level) do
    proof = get_last_claimed_proof_by_address(address)
    if proof == nil do
      Logger.error("No claimed proof found for address #{address}. Cannot update level reached.")
      {:error, :no_claimed_proof_found}
    else
      proof_id = proof.id
      proof = get_proof!(proof_id)
      changeset = change_proof(proof, %{level_reached: level})
      case Repo.update(changeset) do
        {:ok, updated_proof} ->
          Logger.info("Updated level reached for proof #{proof_id} to #{level}")
          {:ok, updated_proof}

        {:error, changeset} ->
          Logger.error("Failed to update level reached for proof #{proof_id}: #{inspect(changeset)}")
          {:error, changeset}
      end
    end
  end
