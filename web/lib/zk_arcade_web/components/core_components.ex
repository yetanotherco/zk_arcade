defmodule ZkArcadeWeb.CoreComponents do
    ...
    <nav class="w-full flex justify-between items-center">
      <div class="w-full flex gap-12 items-center">
        ...
        <div class="lg:flex hidden w-full gap-10">
          ...
          <.link href="/leaderboard" class="transition hover:text-accent-100 hover:underline">
            Leaderboard
          </.link>
          <.link href="/history" class="transition hover:text-accent-100 hover:underline">
            Profile
          </.link>
      ...
      <div class="flex gap-6 items-center">
        ...
        >
          <div class="h-dvh flex flex-col gap-y-10 text-2xl justify-end items-center p-12">
            <.link href="/" class="text-text-100 transition hover:text-accent-100 hover:underline">
              Home
            </.link>
            <.link
              href="/games"
              class="text-text-100 transition hover:text-accent-100 hover:underline"
            >
              Games
            </.link>
            <.link
            ...
            <p
              class="transition hover:text-accent-100 hover:underline cursor-pointer"
              id="how-it-works-nav-btn"
            >
              Tutorial
            </p>
            <.link href="#faq" class="text-text-100 transition hover:text-accent-100 hover:underline">
              FAQ
            </.link>
            <.link
              href={Application.get_env(:zk_arcade, :feedback_form_url)}
  ...
  def faq_entry(assigns) do
    ...
    <div
      class={[
        "scroll-observer transition-[opacity,border] duration-500 hover:opacity-100 group-hover:opacity-50 border-t border-contrast-100 hover:border-accent-100/30",
        @class
      ]}
      data-entry="fade-in-up"
    >
      <div
  ...
  def tag(assigns) do
    ...
    <%= case @variant do %>
      <% :risc0 -> %>
        <p class="mt-2 text-xs text-text-100 bg-risc0/20 border border-risc0 rounded w-fit px-3 font-bold">
          Risc0
        </p>
      <% :cli -> %>
        <p class="mt-2 text-xs text-text-100 bg-orange/20 border border-orange rounded w-fit px-3 font-bold">
          CLI
        </p>
      <% :browser -> %>
        <p class="mt-2 text-xs text-text-100 bg-blue/20 border border-blue rounded w-fit px-3 font-bold">
          Browser
        </p>
      <% :circom -> %>
        <p class="mt-2 text-xs text-text-100 bg-circom/20 border border-circom rounded w-fit px-3 font-bold">
          Circom
        </p>
      <% :sp1 -> %>
        <p class="mt-2 text-xs text-text-100 bg-sp1/20 border border-sp1 rounded w-fit px-3 font-bold">
          SP1
        </p>
      <% _ -> %>
    <% end %>
  ...
  def table(assigns) do
    ...
    <table class="table-fixed border-collapse w-full">
      ...
      <tbody id={@id}>
        ...
        >
          <td :for={{col, _i} <- Enum.with_index(@col)} class={classes(["p-0 pr-10", col[:class]])}>
            <div class={
              classes([
            ...
            <%= for {title, links} <- @headers do %>
              <div class="flex flex-col items-start gap-2">
                ...
                <%= for {value, link} <- links do %>
                  <.link
                    class="text-sm text-text-200 hover:underline"
                    href={link}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
  ...
  def pagination_info(assigns) do
    ...
    <div class="text-center mt-4 text-text-200">
      Showing {(@pagination.current_page - 1) * @items_per_page + 1}-{min(
        @pagination.current_page * @items_per_page,
    ...
    <%= if length(@users) > 0 do %>
      <div class="w-full">
        <div class="overflow-x-auto">
          <.leaderboard_table id="leaderboard" users={@users} current_wallet={@current_wallet} />
          ...
          <a href="/leaderboard">
            <div class="hidden md:block cursor-pointer inline-flex items-center space-x-2 group">
              ...
              <.icon
                ...
                class="size-7 transition-transform duration-200 group-hover:translate-x-1"
              />
            </div>
          </a>
        </div>
