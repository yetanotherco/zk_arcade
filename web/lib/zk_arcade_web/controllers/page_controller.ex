defmodule ZkArcadeWeb.PageController do
  ...
  def home(conn, _params) do
    ...
    eth_price =
      case ZkArcade.EthPrice.get_eth_price_usd() do
        {:ok, price} ->
          price
    ...
    faqs = [
      %{
        ...
        answer:
          "Your wallet is your identity in ZK Arcade. It allows you to deposit ETH into Aligned to pay for proof verification, claim your points on the leaderboard, and keep all your progress securely tied to your address."
      },
      %{
        number: "02",
        question: "Why do I need to verify my proofs?",
        answer:
          "Verification guarantees that your results are valid and are not tampered with. It ensures the leaderboard reflects the player's real skills, and not manipulated outcomes."
      },
      %{
        number: "03",
        question: "Can I resubmit a proof for a lower level?",
        answer:
          "No. You can only submit one proof per level, and each new proof must be for a higher level than the last one submitted for the daily game."
      },
      %{
        number: "04",
        question: "What happens if my proof submission fails?",
        answer:
          "If a proof fails verification, it won’t count toward your score. You can generate a new valid proof and resubmit."
      },
      %{
        number: "05",
        question: "How do I earn points on the leaderboard?",
        answer:
          "Points are awarded per verified level on each day. The higher the level you submit a valid proof for, the more points you receive for it."
      },
      %{
        number: "06",
        question: "Is my gameplay data public?",
        answer:
          "No. Only your proof and score are submitted. Zero-knowledge proofs allow verification without exposing your full gameplay data."
      },
      %{
        number: "07",
      ...
      %{
        ...
        answer:
          "Browser-based games will be mobile-friendly, but Beast 1984 game requires a desktop environment with a bash terminal."
      }
    ]
  ...
  def game(conn, %{"name" => "beast"}) do
    ...
    acknowledgements = [
      %{
        text: "Original Beast game repository",
        link: "https://github.com/dominikwilkowski/beast"
      },
  ...
  def game(conn, %{"name" => "parity"}) do
    ...
    |> assign(:game, %{
      ...
      full_desc:
        "The game is played by moving a cursor with WASD around the board to select different squares in a grid. Each time you select a select a cell by moving the cursor with the arrow keys, the number inside that cell increases by one.
...
",
  ...
  def history(conn, _params) do
    case get_wallet_from_session(conn) do
      nil ->
        conn |> redirect(to: build_redirect_url(conn, "user-not-connected"))
      ...
      wallet ->
        ...
        |> assign(
          ...
          Application.get_env(:zk_arcade, :payment_service_address)
        )
        |> assign(:username, username)
        |> assign(:user_position, position)
        |> assign(:explorer_url, explorer_url)
